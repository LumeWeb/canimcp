...
// a wire request to a Profile. Detectors are tried in priority
...
// Detect resolves a Evidence to a Profile. It:
...
func (r *DetectorRegistry) Detect(req Evidence) Profile {
...
// DetectFromHTTPRequest is a convenience that builds a Evidence
// from an HTTP request and the server's transport flags, then calls
// Detect.
func (r *DetectorRegistry) DetectFromHTTPRequest(h http.Header, coLocated, tunnelOpenAI bool, tokenInfo *TokenInfo) Profile {
	req := Evidence{
...
func detectTransport(req Evidence) TransportKind {
	if req.CoLocated {
