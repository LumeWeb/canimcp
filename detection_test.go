...
func TestOpenCodeDetector_NoMatch(t *testing.T) {
	...
	host, auth = d.Match(Evidence{CoLocated: true})
	require.Equal(t, HostUnknown, host)
	require.Equal(t, AuthMethod(""), auth)
...
func TestClaudeDetector_MatchByUserAgent(t *testing.T) {
	...
	host, auth := d.Match(Evidence{UserAgent: "Claude-User"})
...
func TestClaudeDetector_MatchByUserAgentWithToken(t *testing.T) {
	...
	host, auth := d.Match(Evidence{UserAgent: "Claude-User", TokenInfo: newTokenInfo()})
	require.Equal(t, HostClaude, host)
	require.Equal(t, AuthOAuth, auth)
}
...
func TestClaudeDetector_BareHeaderDoesNotMatch(t *testing.T) {
	...
	host, auth := d.Match(Evidence{Headers: h})
	require.Equal(t, HostUnknown, host)
	require.Equal(t, AuthMethod(""), auth)

	h2 := http.Header{}
	h2.Set("X-Anthropic-Version", "2026-07-28")
	host, auth = d.Match(Evidence{Headers: h2})
	require.Equal(t, HostUnknown, host)
	require.Equal(t, AuthMethod(""), auth)
}
...
func TestClaudeDetector_NarrowUA(t *testing.T) {
	...
	host, auth := d.Match(Evidence{UserAgent: "claude-connector/1.0"})
	require.Equal(t, HostUnknown, host)
	require.Equal(t, AuthMethod(""), auth)
}
...
func TestClaudeDetector_NoMatch(t *testing.T) {
	...
	// Unrelated signals
	host, auth := d.Match(Evidence{UserAgent: "some-random-client/2.0.0"})
	require.Equal(t, HostUnknown, host)
	require.Equal(t, AuthMethod(""), auth)

	// Claude Web is always remote — co-located is handled by claudeDesktopDetector.
	host, auth = d.Match(Evidence{UserAgent: "Claude-User", CoLocated: true})
	require.Equal(t, HostUnknown, host)
	require.Equal(t, AuthMethod(""), auth)

	// Claude Web never uses the OpenAI tunnel.
	host, auth = d.Match(Evidence{UserAgent: "Claude-User", TunnelOpenAI: true})
	require.Equal(t, HostUnknown, host)
	require.Equal(t, AuthMethod(""), auth)
}
...
func TestClaudeDesktopDetector_NoMatch(t *testing.T) {
	...
	host, auth := d.Match(Evidence{ClientInfo: &ClientInfo{Name: "claude-ai"}})
	require.Equal(t, HostUnknown, host)
	require.Equal(t, AuthMethod(""), auth)
...
func TestDetectTransport(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			req:  Evidence{CoLocated: true},
			want: TransportStdio,
		},
		{
			name: "co-located takes priority over tunnel → stdio",
			req:  Evidence{CoLocated: true, TunnelOpenAI: true},
			want: TransportStdio,
		},
		{
			name: "tunnel openai → openai",
			req:  Evidence{TunnelOpenAI: true},
		...
		},
		{
			...
			req:  Evidence{CoLocated: false, TunnelOpenAI: false},
			want: TransportHTTP,
		},
	}
