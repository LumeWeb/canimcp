...
// newProfile assembles a static Profile by merging a host's declared
...
// they do NOT widen the declarative source.mode schema enum, which
// travels only with the transport (via TransportKindFromFeatures). Declaring a
// source mechanism feature never implies the corresponding declarative source
// mode.
func newProfile(caps FeatureSet, host HostType, t TransportKind, auth AuthMethod, remote bool) Profile {
	features := caps.Clone()
...
// cannot hand the server an OpenAI {download_url, file_id} file object (no
...
// and drives their positive description copy. It does NOT flip the primary
...
// declarative upload schema to url/data modes on HTTP — the separate
...
// ProfileGrokStdio is xAI Grok Shell (clientInfo name "grok-shell-agent" and
...
func ProfileForTransport(t TransportKind) Profile {
	switch t {
	case TransportStdio:
